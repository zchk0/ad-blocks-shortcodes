...
class ABS_Ad_Blocks_Rotator {
    ...
    public function __construct() {
        ...
        add_action('admin_footer', [$this, 'admin_footer_inline_js']);
    ...
    public function register_cpts() {
        register_post_type(self::CPT_GROUP, [
            'labels' => [
                'name'          => 'Реклама: группы',
                'singular_name' => 'Группа рекламы',
                'add_new_item'  => 'Добавить группу',
                'edit_item'     => 'Редактировать группу',
                'menu_name'     => 'Реклама (шорткоды)',
            ],
            'public'             => false,
            'show_ui'            => true,
            'show_in_menu'       => true,
    ...
    public function admin_assets() {
        ...
        if (in_array($screen->post_type, [self::CPT_ITEM, self::CPT_GROUP], true)) {
            // нужно для выбора/загрузки картинок
            wp_enqueue_media();
        }
    }

    public function frontend_assets() {
        // Базовый CSS, чтобы картинки не вылезали из блока.
        // Индивидуальные настройки размеров применяются inline-style на img.
        $css = '
            .abs-ad-block, .abs-ad-block * { box-sizing: border-box; }
            .abs-ad-inner { width:100%; overflow:hidden; }
            .abs-ad-img-wrap { width:100%; overflow:hidden; }
            .abs-ad-img-wrap img { max-width:100%; height:auto; display:block; }
            .abs-ad-block a { display:block; }
        ';
        wp_register_style('abs-ad-rotator-inline', false);
        wp_enqueue_style('abs-ad-rotator-inline');
        wp_add_inline_style('abs-ad-rotator-inline', $css);
    }

    public function admin_footer_inline_js() {
        $screen = function_exists('get_current_screen') ? get_current_screen() : null;
        ...
        <script>
          jQuery(function($){
            ...
            $('#abs_pick_image').on('click', function(e){
              e.preventDefault();
            ...
            $('#abs_clear_image').on('click', function(e){
              e.preventDefault();
